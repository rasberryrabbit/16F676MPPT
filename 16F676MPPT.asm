...
L__main27:
;16F676MPPT.mpas,186 :: 		doADCRead:=0;
	CLRF       _doADCRead+0
;16F676MPPT.mpas,188 :: 		vol_prev2:=vol_prev1;
	MOVF       _vol_prev1+0, 0
	MOVWF      _vol_prev2+0
	MOVF       _vol_prev1+1, 0
	MOVWF      _vol_prev2+1
;16F676MPPT.mpas,189 :: 		vol_prev1:=adc_vol;
	MOVF       _adc_vol+0, 0
	MOVWF      _vol_prev1+0
	MOVF       _adc_vol+1, 0
	MOVWF      _vol_prev1+1
...
;16F676MPPT.mpas,197 :: 		adc_vol:=(adc_vol+wtmp) div 2;
	...
	RRF        _adc_vol+0, 1
	BCF        _adc_vol+1, 7
;16F676MPPT.mpas,198 :: 		if xtmp > adc_cur then
	MOVF       _xtmp+1, 0
	SUBWF      _adc_cur+1, 0
	BTFSS      STATUS+0, 2
	GOTO       L__main73
...
L__main73:
	BTFSC      STATUS+0, 0
	GOTO       L__main40
;16F676MPPT.mpas,199 :: 		adc_cur := xtmp;
	MOVF       _xtmp+0, 0
	MOVWF      _adc_cur+0
	MOVF       _xtmp+1, 0
	MOVWF      _adc_cur+1
L__main40:
;16F676MPPT.mpas,201 :: 		end;
	MOVF       _i+0, 0
	XORLW      4
	BTFSC      STATUS+0, 2
	GOTO       L__main38
	...
	GOTO       L__main35
L__main38:
;16F676MPPT.mpas,202 :: 		adc_vol:=adc_vol * VOLMUL;
	RLF        _adc_vol+0, 1
	RLF        _adc_vol+1, 1
	BCF        _adc_vol+0, 0
	RLF        _adc_vol+0, 1
	RLF        _adc_vol+1, 1
	BCF        _adc_vol+0, 0
;16F676MPPT.mpas,205 :: 		wtmp:=TICK_1000;
	MOVF       _TICK_1000+0, 0
	MOVWF      _wtmp+0
	MOVF       _TICK_1000+1, 0
	MOVWF      _wtmp+1
;16F676MPPT.mpas,206 :: 		if wtmp - powertime < _UPDATE_INT then
	MOVF       _powertime+0, 0
	SUBWF      _TICK_1000+0, 0
	MOVWF      R1+0
	MOVF       _powertime+1, 0
...
L__main74:
	BTFSC      STATUS+0, 0
	GOTO       L__main43
;16F676MPPT.mpas,207 :: 		goto CONTLOOP;
	GOTO       L__main_CONTLOOP
L__main43:
;16F676MPPT.mpas,208 :: 		clrwdt;
	CLRWDT
;16F676MPPT.mpas,209 :: 		powertime:=wtmp;
	MOVF       _wtmp+0, 0
	MOVWF      _powertime+0
	MOVF       _wtmp+1, 0
	MOVWF      _powertime+1
;16F676MPPT.mpas,211 :: 		power_prev:= power_curr;
	...
	MOVWF      _power_prev+3
;16F676MPPT.mpas,212 :: 		power_curr:= adc_vol * adc_cur;
	MOVF       _adc_vol+0, 0
	MOVWF      R0+0
	MOVF       _adc_vol+1, 0
	MOVWF      R0+1
	...
	MOVF       R0+3, 0
	MOVWF      _power_curr+3
;16F676MPPT.mpas,215 :: 		wtmp:=TICK_1000;
	MOVF       _TICK_1000+0, 0
	MOVWF      _wtmp+0
	MOVF       _TICK_1000+1, 0
	MOVWF      _wtmp+1
;16F676MPPT.mpas,216 :: 		if ON_PWM>PWM_MID then begin
	MOVF       _ON_PWM+0, 0
	SUBLW      110
	BTFSC      STATUS+0, 0
	GOTO       L__main46
;16F676MPPT.mpas,217 :: 		if wtmp - voltime > _PWM_CHECK then begin
	MOVF       _voltime+0, 0
	SUBWF      _wtmp+0, 0
	MOVWF      R1+0
	MOVF       _voltime+1, 0
...
L__main75:
	BTFSC      STATUS+0, 0
	GOTO       L__main49
;16F676MPPT.mpas,218 :: 		voltime:=wtmp;
	MOVF       _wtmp+0, 0
	MOVWF      _voltime+0
	MOVF       _wtmp+1, 0
	MOVWF      _voltime+1
;16F676MPPT.mpas,219 :: 		adc_cur:=LM358_diff;
	MOVF       _LM358_diff+0, 0
	MOVWF      _adc_cur+0
	CLRF       _adc_cur+1
;16F676MPPT.mpas,220 :: 		end;
L__main49:
;16F676MPPT.mpas,221 :: 		end else
	GOTO       L__main47
L__main46:
;16F676MPPT.mpas,222 :: 		voltime:=wtmp;
	MOVF       _wtmp+0, 0
	MOVWF      _voltime+0
	MOVF       _wtmp+1, 0
	MOVWF      _voltime+1
L__main47:
;16F676MPPT.mpas,224 :: 		if adc_cur>LM358_diff then begin
	MOVF       _adc_cur+1, 0
	SUBLW      0
	BTFSS      STATUS+0, 2
	GOTO       L__main76
...
L__main76:
	BTFSC      STATUS+0, 0
	GOTO       L__main52
;16F676MPPT.mpas,226 :: 		if power_curr = power_prev then begin
	MOVF       _power_curr+3, 0
...
L__main77:
	BTFSS      STATUS+0, 2
	GOTO       L__main55
;16F676MPPT.mpas,227 :: 		LED1_tm:=250;
	MOVLW      250
	MOVWF      _LED1_tm+0
;16F676MPPT.mpas,228 :: 		goto CONTLOOP;
	GOTO       L__main_CONTLOOP
;16F676MPPT.mpas,229 :: 		end else if power_curr < power_prev then begin
L__main55:
...
;16F676MPPT.mpas,231 :: 		flag_inc:=not flag_inc;
	COMF       _flag_inc+0, 1
;16F676MPPT.mpas,232 :: 		end else
	GOTO       L__main59
L__main58:
;16F676MPPT.mpas,233 :: 		LED1_tm:=200;
	MOVLW      200
	MOVWF      _LED1_tm+0
L__main59:
;16F676MPPT.mpas,239 :: 		end else begin
	GOTO       L__main53
L__main52:
;16F676MPPT.mpas,240 :: 		LED1_tm:=100;
	MOVLW      100
	MOVWF      _LED1_tm+0
;16F676MPPT.mpas,241 :: 		VOLPWM:=PWM_MID;
	MOVLW      110
	MOVWF      _VOLPWM+0
;16F676MPPT.mpas,242 :: 		flag_inc:=false;
	CLRF       _flag_inc+0
;16F676MPPT.mpas,243 :: 		power_curr:=0;
	CLRF       _power_curr+0
	CLRF       _power_curr+1
	CLRF       _power_curr+2
	CLRF       _power_curr+3
;16F676MPPT.mpas,244 :: 		adc_cur:=0;
	CLRF       _adc_cur+0
	CLRF       _adc_cur+1
;16F676MPPT.mpas,245 :: 		goto CONTLOOP;
	GOTO       L__main_CONTLOOP
;16F676MPPT.mpas,246 :: 		end;
L__main53:
;16F676MPPT.mpas,249 :: 		if flag_inc then begin
	MOVF       _flag_inc+0, 0
	BTFSC      STATUS+0, 2
	GOTO       L__main61
;16F676MPPT.mpas,250 :: 		if VOLPWM<PWM_MAX then
	MOVLW      230
	SUBWF      _VOLPWM+0, 0
	BTFSC      STATUS+0, 0
	GOTO       L__main64
;16F676MPPT.mpas,251 :: 		Inc(VOLPWM)
	INCF       _VOLPWM+0, 1
	GOTO       L__main65
;16F676MPPT.mpas,252 :: 		else begin
L__main64:
;16F676MPPT.mpas,253 :: 		VOLPWM:=PWM_MAX;
	MOVLW      230
	MOVWF      _VOLPWM+0
;16F676MPPT.mpas,254 :: 		flag_inc:=false;
	CLRF       _flag_inc+0
;16F676MPPT.mpas,255 :: 		end;
L__main65:
;16F676MPPT.mpas,256 :: 		end else begin
	GOTO       L__main62
L__main61:
;16F676MPPT.mpas,257 :: 		if VOLPWM>PWM_MIN then
	MOVF       _VOLPWM+0, 0
	SUBLW      1
	BTFSC      STATUS+0, 0
	GOTO       L__main67
;16F676MPPT.mpas,258 :: 		Dec(VOLPWM)
	DECF       _VOLPWM+0, 1
	GOTO       L__main68
;16F676MPPT.mpas,259 :: 		else begin
L__main67:
;16F676MPPT.mpas,260 :: 		VOLPWM:=PWM_MIN;
	MOVLW      1
	MOVWF      _VOLPWM+0
;16F676MPPT.mpas,261 :: 		flag_inc:=true;
	MOVLW      255
	MOVWF      _flag_inc+0
